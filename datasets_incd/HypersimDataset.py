...
def intrinsic2incidence(K, b, h, w, device=None):
    ...
    # return img
    return pts3d.squeeze().permute(2,0,1), img_array
...
class HypersimDataset:
    def __init__(self,
        ...
        json_folder = "/mnt/nas/share2/home/xugk/marigold_github_repo/datasets_20231223/hypersim/depth/jsons/"
        ...
        self.root = '/mnt/nas/share2/home/xugk/marigold_github_repo/datasets_20231223'
        self.data = []
        for json_path in json_files:
            with open(json_path, 'r') as f:
                for line in f:
                    ...
                    # mask_path = image_path.replace('rgb', 'seg').replace('tonemap', 'seg')
        ...
        # self.transform = transforms.ResizeLongestSide(1024)

        self.data_names = 'Hypersim'
    ...
    def __getitem__(self, idx):
        ...
        # data_dict = {
        #     'rgb_raw': os.path.join(self.root, image_path),
        # }
        # return data_dict
        rgb = self.load_im(os.path.join(self.root, image_path))
        normal = self.load_im(os.path.join(self.root, normal_image_path))
        # mask = self.load_im(os.path.join(self.root, mask_path))
        mask = normal
        fx, fy, u, v = 886.81, 886.81, 1024.0*0.5, 768.0*0.5
        ...
        input_image_torch = torch.as_tensor(np.array(image)).permute(2, 0, 1).contiguous()
        ...
        mask = mask.resize((self.wt, self.ht))
        ...
        mask = self.normalize(self.tensor(mask))
        ...
        if self.augmentation:
            if self.training:
                rgb, K, T, normal, mask = apply_augmentation(
                    rgb, K, normal, mask, seed=None, augscale=self.augscale, no_change_prob=self.no_change_prob,
                )
            else:
        ...
        # K to incident field
        # print(K)
        incidence_gt, incident_img = intrinsic2incidence(K, 1, self.wt, self.ht, rgb.device)
