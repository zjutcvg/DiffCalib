...
def pcd_uv_rgb_mask_to_sparsetensor(pcd, rgb, mask_feature, u_u0, v_v0, mask_valid, f= 500.0, voxel_size=0.01, mask_side=None, num_points=100000):
    ...
    rgb = rgb.permute(2, 1, 0) # H W C
    rgb_valid = rgb[mask_valid]
    mask_feature_valid = mask_feature[mask_valid]

    block_ = np.concatenate([pcd_valid, u_u0_valid, v_v0_valid, rgb_valid, mask_feature_valid], axis=1)
    ...
    return inputs
def pcd_uv_rgb_to_sparsetensor(pcd, rgb, u_u0, v_v0, mask_valid, f= 500.0, voxel_size=0.01, mask_side=None, num_points=100000):
    if mask_side is not None:
        mask_valid = mask_valid & mask_side
    pcd_valid = pcd[mask_valid]
    u_u0_valid = u_u0[mask_valid][:, np.newaxis] / f
    ...
    block_ = np.concatenate([pcd_valid, u_u0_valid, v_v0_valid, rgb_valid], axis=1)
    block = np.zeros_like(block_)
    block[:, :] = block_[:, :]
