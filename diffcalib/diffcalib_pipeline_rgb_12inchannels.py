...
class DiffcalibTwoOutput(BaseOutput):
...
class DiffcalibPipeline(DiffusionPipeline):
    """
    Pipeline for monocular depth estimation and intrinsic estimation.
    ...
    def __call__(
        ...
        for batch in iterable:
            (batched_img,) = batch
            incident_pred_raw, depth_pred_raw = self.single_infer(
                rgb_in=batched_img,
        ...
        if mode == 'depth':
            ...
            depth = depth_preds.clip(-1, 1).cpu().numpy().astype(np.float16) # [-1, 1] [3, h, w]
            # depth_pred = depth_pred.cpu().numpy().astype(np.float32)
            ...
            depth_colored = norm_to_rgb(depth)
            depth_colored_hwc = chw2hwc(depth_colored)
            ...
            depth_pred = np.asarray(depth_colored_img) / 255.0 * 2.0 - 1.0
            # import pdb
            # pdb.set_trace()
            return DiffcalibTwoOutput(
                incident_np=np.transpose(incident_pred, (2, 0, 1)),
                incident_colored=incident_colored_img,
                depth_np=np.transpose(depth_pred, (2, 0, 1)),
                depth_colored=depth_colored_img,
                uncertainty=None,
            )
    ...
    def single_infer(
        ...
        for i, t in iterable:
            unet_input = torch.cat(
                [rgb_latent,depth_latent,incident_latent], dim=1
            ...
            noise_pred_depth = noise_pred[:, 4:]

            # compute the previous noisy sample x_t -> x_t-1
            # print('===3===')
            incident_latent = self.scheduler.step(noise_pred_incident, t, incident_latent).prev_sample
            depth_latent = self.scheduler.step(noise_pred_depth, t, depth_latent).prev_sample
        ...
        if mode == 'depth':
            ...
            # shift to [0, 1]
            depth = (depth + 1.0) / 2.0
            incident_map = self.decode_depth(incident_latent)
            ...
            incident_map = self.decode_depth(incident_latent)
            depth_map = self.decode_depth(depth_latent)
            incident_map = torch.clip(incident_map, -1.0, 1.0)
            depth_map = torch.clip(depth_map, -1.0, 1.0)
            return incident_map, depth_map
        else:
            raise NotImplementedError
    ...
    def decode_depth(self, depth_latent: torch.Tensor) -> torch.Tensor:
        ...
        """
        # scale latent
        depth_latent = depth_latent / self.seg_latent_scale_factor
        # decode
